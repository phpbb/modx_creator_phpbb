...
$imageset_path = "{$web_path}styles/" . $user->theme['imageset_path'] . '/imageset';
...
if ($submit_file && $modx_data != '' && !$submit)
{
	if ($submit_file == MODX)
	{
		include($phpbb_root_path . 'includes/modx_creator_phpbb/modx_parser.' . $phpEx);
		$parser = new modx_parser($modx_data);
	}
	else if ($submit_file == MOD)
	{
		include($phpbb_root_path . 'includes/modx_creator_phpbb/mod_parser.' . $phpEx);
...
if (!isset($parser))
{
	include($phpbb_root_path . 'includes/modx_creator_phpbb/post_parser.' . $phpEx);
				...
				foreach ($value2 as $key3 => $value3)
				{
					...
					if($value3['type'] != '' && isset($value3['data']))
					{
						...
						if(strpos($value3['type'], 'inline') !== FALSE)
						{
							...
							$modx_img = '<img id="' . $dd_id . '_info" class="action-image" src="' . $imageset_path . '/modx_info.png" alt="Inline explain" title="Note that the inline tags may not contain line breaks" />';
						}
